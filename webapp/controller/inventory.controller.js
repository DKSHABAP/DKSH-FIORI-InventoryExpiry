sap.ui.define(["sap/ui/core/mvc/Controller", "sap/ui/model/json/JSONModel", "sap/m/MessageBox", "sap/ui/core/Fragment",
	...
	"use strict";
	return e.extend("dksh.connectclient.InventoryExp.controller.inventory", {
		formatter: o,
		...
		_getUser: function () {
			...
			this._doAjax(e, "GET", "", true).then(e => {
				...
				this.clearTabData();
				this._getPersonalizationDetails("keyExpiryStockView");
		...
		onPressDetailEndStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryExp.Fragments.EndingStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
		...
		onPressDetailExpStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryExp.Fragments.ExpiryStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailMatMov: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryExp.Fragments.MatMovPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailATP: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryExp.Fragments.ATPPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onTabSelection: function (e) {
			...
			this.selectedTab = "keyExpiryStockView";
			this.clearTabData();
		...
		onPersonlizationClose: function () {
			...
			if (!this.FilterPersonalization) {
				this.FilterPersonalization = sap.ui.xmlfragment("dksh.connectclient.InventoryExp.Fragments.FilterPersonalization", this);
			...
			// e._getPersonalizationDetails(this.selectedTab)
			e._getPersonalizationDetails("keyExpiryStockView")
		},
		onVariantOK: function () {
			var e = this;
			var t = this.getView().getModel("PersonalizationModel");
			if (!this.FilterPersonalization) {
				this.FilterPersonalization = sap.ui.xmlfragment("incture.com.InventoryExp.Fragments.FilterPersonalization", this);
				this.getView().addDependent(this.FilterPersonalization)
			}
			var s = new sap.ui.model.json.JSONModel({
				results: this.getView().getModel("PersonalizationModel").getData()
		...
		valueHelpRequestPlant: function (e) {
			...
			if (t.plantDataAccess === "No Access") {
				...
				var t = this;
				if (!t.plantFrag) {
					t.plantFrag = sap.ui.xmlfragment("dksh.connectclient.InventoryExp.Fragments.plant", t);
					...
					var l = new sap.ui.model.Filter({
						filters: [new sap.ui.model.Filter("language", sap.ui.model.FilterOperator.EQ, o), new sap.ui.model.Filter("plant", sap.ui.model
							.FilterOperator.EQ, this.plantDataAccess)],
						and: true
					});
					r.push(l);
		...
		valueHelpRequestMaterialGrp4: function (e) {
			...
			if (t.materialGroup4DataAccess === "No Access") {
				...
				if (!t.materialGroup4) {
					t.materialGroup4 = sap.ui.xmlfragment("incture.com.InventoryExp.Fragments.materialGroup4", t);
		...
		valueHelpRequestMaterialGrp: function (e) {
			...
			if (t.materialGroupDataAccess === "No Access") {
				...
				if (!t.MaterialGroup) {
					t.MaterialGroup = sap.ui.xmlfragment("dksh.connectclient.InventoryExp.Fragments.MaterialGroup", t);
		...
		valueHelpRequestStoLoc: function (e) {
			...
			if (l.SLOCDataAccess === "No Access") {
				...
				l.StorageLocation = sap.ui.xmlfragment("incture.com.InventoryExp.Fragments.StorageLocation", l);
		...
		onVariantDelete: function () {
			...
			var o = {
				...
				applicationId: "keyExpiryStockView",
		...
		onLiveChangePlant: function (e) {
			var t = e.getParameters().value;
			var s = new Array;
			var r = new sap.ui.model.Filter([new sap.ui.model.Filter("plantName", sap.ui.model.FilterOperator.Contains, t), new sap.ui.model.Filter(
				"plant", sap.ui.model.FilterOperator.Contains, t)]);
			s.push(r);
			var a = e.getSource().getBinding("items");
			a.filter(s)
		},
			...
			for (var s = 0; s < r.length; s++) {
				if (r[s].status === true) {
					if (r[s].enabledKey === "Material Group") {
						...
						a.push(l)
					} else if (r[s].enabledKey === "SLocDesc") {
						var l = {
		...
		popOver: function (e) {
			...
			if (!this._oPopover) {
				r.load({
					name: "incture.com.InventoryExp.Fragments.popoverTable",
					controller: this
				}).then(function (e) {
					this._oPopover = e;
					this.getView().addDependent(this._oPopover);
		...
		navigateStkLot: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryExp.Fragments.StockLotPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		valueHelpRequestSalesOrg: function (e) {
			...
			if (t.salesOrgDataAccess === "No Access") {
				...
				if (!t.salesOrg) {
					t.salesOrg = sap.ui.xmlfragment("dksh.connectclient.InventoryExp.Fragments.SalesOrg", t);
